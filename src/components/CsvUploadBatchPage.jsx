...
// Pantalla genérica de "sube un CSV grande (400k+ filas), se procesa en chunks en memoria
// (nunca toca disco), con historial de lotes y borrado en background" — usada por Existencia
// CeVe Manual, PedidoVSCargo Real, Promedios de Pedido y Participación - Tipo Movimiento.
export default function CsvUploadBatchPage({ apiPath, title, description, csvCols, templateFileName, templateSampleRow }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API}${apiPath}/batches`).then(r => r.ok ? r.json() : []).then(list => {
      setBatches(list)
      setLoadingB(false)
  ...
  function handleDownloadTemplate() {
    ...
    const content = templateSampleRow ? `${header}\n${templateSampleRow}\n` : `${header}\n`
  ...
  const CHUNK_SIZE = 32 * 1024 * 1024 // 32MB por request — un solo POST con el CSV completo
                                      // (puede pesar GBs con 400k+ filas) se pasa del timeout de
                                      // Azure aunque el servidor procese rápido, así que se sube
                                      // en pedazos.
  const MAX_RETRIES = 4
  ...
  async function handleUpload() {
    ...
    try {
      const initR = await fetchWithRetry(`${API}${apiPath}/upload/init?fileName=${encodeURIComponent(file.name)}`, { method: 'POST' })
      if (!initR.ok) throw new Error(`HTTP ${initR.status} al iniciar la subida`)
      const { uploadId } = await initR.json()

      for (let offset = 0; offset < file.size; offset += CHUNK_SIZE) {
        const chunk = file.slice(offset, offset + CHUNK_SIZE)
        const r = await fetchWithRetry(`${API}${apiPath}/upload/chunk?uploadId=${uploadId}&expectedOffset=${offset}`, {
      ...
      const compR = await fetchWithRetry(`${API}${apiPath}/upload/complete?uploadId=${uploadId}`, { method: 'POST' })
  ...
  async function handleDelete(batchId, label) {
    ...
    setDeleting(batchId)
    try {
      const r = await fetch(`${API}${apiPath}/batches/${batchId}`, { method: 'DELETE' })
