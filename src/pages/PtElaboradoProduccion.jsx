...
const COLS = [
  'semana', 'org', 'work_area_name', 'item_number', 'description', 'uom_code', 'elaboratedproduct',
  'production_value', 'scrappieces', 'scrap_value', 'per_scrap', 'cost_per_unit', 'tons_produced',
  'linea nva', 'planta', 'anio', 'tipo', 'categoria', 'subcategoria',
]
const EJEMPLO = [
  '3', 'ORG1', 'Linea1', '10001', 'Pan Blanco Grande', 'PZA', 'SI',
  '10000', '0', '0', '0', '1.50', '5.20',
  'L1', 'PlantaX', '2026', 'PT', 'Panes', 'Blanco',
]
const UPLOAD_URL  = '/api/pt-elaborado-produccion/upload'
const BATCHES_URL = '/api/pt-elaborado-produccion/batches'
const DELETE_URL  = '/api/pt-elaborado-produccion/batches'
...
function descargarPlantilla() {
  ...
  a.href = url; a.download = 'plantilla_pt_elaborado_produccion.csv'
  document.body.appendChild(a); a.click(); document.body.removeChild(a)
  URL.revokeObjectURL(url)
}

const ROWS_COLS = [
  ['anio', 'anio'], ['semana', 'semana'], ['org', 'org'], ['workAreaName', 'work_area_name'],
  ['itemNumber', 'item_number'], ['description', 'description'], ['uomCode', 'uom_code'],
  ['elaboratedProduct', 'elaboratedproduct'], ['productionValue', 'production_value'],
  ['scrapPieces', 'scrappieces'], ['scrapValue', 'scrap_value'], ['perScrap', 'per_scrap'],
  ['costPerUnit', 'cost_per_unit'], ['tonsProduced', 'tons_produced'], ['lineaNva', 'linea nva'],
  ['planta', 'planta'], ['tipo', 'tipo'], ['categoria', 'categoria'], ['subcategoria', 'subcategoria'],
]
...
async function exportarBatchExcel(batchId, nombreArchivo) {
  ...
  const ws = wb.addWorksheet('PT Elaborado Producción', { views: [{ state: 'frozen', ySplit: 1 }] })
  ...
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url; a.download = `${(nombreArchivo || 'pt_elaborado_produccion').replace(/\.csv$/i, '')}.xlsx`
  document.body.appendChild(a); a.click(); document.body.removeChild(a)
  URL.revokeObjectURL(url)
}
...
export default function PtElaboradoProduccion() {
