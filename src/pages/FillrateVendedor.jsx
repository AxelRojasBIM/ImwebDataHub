...
const COLS = [
  'codigo_ceve', 'fecha_venta', 'semana', 'año', 'codigo_canal', 'item',
  'descarga_pzs', 'devolución_pzs', 'existencia_ruta_pzs', 'faltante_ruta_pzs',
  'faltante_ruta_as', 'rechazos_ruta_as', 'pedido_sin_incidencias_pzs', 'pedido_sin_incidencias_importe',
  'cargo_sin_aumento_pzs', 'cargo_sin_aumento_importe', 'cargo_pzs', 'cargo_importe',
  'recorte_cargo_sin_aumento_menos_pedido_sin_incidencias_pzs',
  'recorte_cargo_sin_aumento_menos_pedido_sin_incidencias_importe',
]
const EJEMPLO = [
  '20279', '2026-09-01', '36', '2026', '560', '12345',
  '100', '5', '0', '0',
  '0', '0', '95', '1900',
  '95', '1900', '100', '2000',
  '-5', '-100',
]
const UPLOAD_URL  = '/api/fillrate-vendedor/upload'
const BATCHES_URL = '/api/fillrate-vendedor/batches'
const DELETE_URL  = '/api/fillrate-vendedor/batches'

const REGLAS = [
  'Ninguna columna puede quedar vacía.',
  'Los 14 campos numéricos (descarga_pzs, devolución_pzs, existencia_ruta_pzs, faltante_ruta_pzs, faltante_ruta_as, rechazos_ruta_as, pedido_sin_incidencias_pzs, pedido_sin_incidencias_importe, cargo_sin_aumento_pzs, cargo_sin_aumento_importe, cargo_pzs, cargo_importe y los dos de recorte) deben ser números válidos — cero y negativos son valores normales.',
  'No se permiten filas duplicadas por codigo_ceve + año + semana + fecha_venta + codigo_canal + item.',
]
...
function descargarPlantilla() {
  ...
  a.href = url; a.download = 'plantilla_fillrate_vendedor.csv'
  document.body.appendChild(a); a.click(); document.body.removeChild(a)
  URL.revokeObjectURL(url)
}

const ROWS_COLS = [
  ['codigoCeve', 'codigo_ceve'], ['fechaVenta', 'fecha_venta'], ['semana', 'semana'], ['anio', 'año'],
  ['codigoCanal', 'codigo_canal'], ['item', 'item'],
  ['descargaPzs', 'descarga_pzs'], ['devolucionPzs', 'devolución_pzs'],
  ['existenciaRutaPzs', 'existencia_ruta_pzs'], ['faltanteRutaPzs', 'faltante_ruta_pzs'],
  ['faltanteRutaAS', 'faltante_ruta_as'], ['rechazosRutaAS', 'rechazos_ruta_as'],
  ['pedidoSinIncidenciasPzs', 'pedido_sin_incidencias_pzs'], ['pedidoSinIncidenciasImporte', 'pedido_sin_incidencias_importe'],
  ['cargoSinAumentoPzs', 'cargo_sin_aumento_pzs'], ['cargoSinAumentoImporte', 'cargo_sin_aumento_importe'],
  ['cargoPzs', 'cargo_pzs'], ['cargoImporte', 'cargo_importe'],
  ['recortePzs', 'recorte_pzs'], ['recorteImporte', 'recorte_importe'],
]
...
async function exportarBatchExcel(batchId, nombreArchivo) {
  ...
  const ws = wb.addWorksheet('Fillrate Vendedor', { views: [{ state: 'frozen', ySplit: 1 }] })
  ...
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url; a.download = `${(nombreArchivo || 'fillrate_vendedor').replace(/\.csv$/i, '')}.xlsx`
  document.body.appendChild(a); a.click(); document.body.removeChild(a)
  URL.revokeObjectURL(url)
}
...
export default function FillrateVendedor() {
