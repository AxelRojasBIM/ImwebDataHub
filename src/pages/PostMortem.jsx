import { useState, useEffect, useRef } from 'react'
import { API } from '../App'

function fmtDur(ms) {
  if (ms == null) return '—'
  if (ms < 1000) return `${ms}ms`
  const s = Math.round(ms / 1000)
  if (s < 60) return `${s}s`
  return `${Math.floor(s / 60)}m ${s % 60}s`
}

export default function PostMortem() {
  const today = new Date().toISOString().slice(0, 10)
  const [fechaVenta, setFechaVenta] = useState(today)
  const [running, setRunning]       = useState(false)
  const [estado, setEstado]         = useState(null)
  const [result, setResult]         = useState(null)
  const [historial, setHistorial]   = useState([])
  const [loadingH, setLoadingH]     = useState(true)
  const [deletingId, setDeletingId] = useState(null)
  const pollRef = useRef(null)

  async function loadHistorial() {
    setLoadingH(true)
    try {
      const r = await fetch(`${API}/api/post-mortem/historial`)
      if (r.ok) setHistorial(await r.json())
    } catch {}
    finally { setLoadingH(false) }
  }

  async function checkEstado() {
    try {
      const r = await fetch(`${API}/api/post-mortem/estado`)
      if (!r.ok) return
      const d = await r.json()
      setEstado(d)
      if (d.estado === 'completado' || d.estado === 'error') {
        clearInterval(pollRef.current)
        setRunning(false)
        if (d.estado === 'completado') setResult({ ok: true, d: d.resultado })
        else setResult({ ok: false, msg: d.error })
        loadHistorial()
      }
    } catch {}
  }

  useEffect(() => {
    loadHistorial()
    checkEstado().then(() => {
      setEstado(prev => {
        if (prev?.estado === 'ejecutando') {
          setRunning(true)
          pollRef.current = setInterval(checkEstado, 3000)
        }
        return prev
      })
    })
    return () => clearInterval(pollRef.current)
  }, [])

  async function handleEjecutar() {
    if (!fechaVenta) return
    if (!confirm(`¿Ejecutar Post-Mortem para la fecha de venta ${fechaVenta}? Necesita una ejecución de Existencia Teórica ya completada para esa fecha.`)) return

    setRunning(true); setResult(null)
    try {
      const r = await fetch(`${API}/api/post-mortem/ejecutar`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ fechaVenta, usuario: 'axel.rojas' }),
      })
      const d = await r.json()
      if (!r.ok) throw new Error(d.detail || d.error || `HTTP ${r.status}`)
      pollRef.current = setInterval(checkEstado, 3000)
    } catch (e) {
      setRunning(false)
      setResult({ ok: false, msg: e.message })
    }
  }

  function handleExportar(ejecucionId) {
    window.open(`${API}/api/post-mortem/exportar?ejecucionId=${encodeURIComponent(ejecucionId)}`, '_blank')
  }

  async function handleEliminar(ejecucionId) {
    if (!confirm('¿Eliminar esta ejecución y todos sus resultados? Esta acción no se puede deshacer.')) return
    setDeletingId(ejecucionId)
    try {
      await fetch(`${API}/api/post-mortem/ejecuciones/${encodeURIComponent(ejecucionId)}`, { method: 'DELETE' })
      await loadHistorial()
    } catch {}
    finally { setDeletingId(null) }
  }

  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 24px' }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0, color: 'var(--text)' }}>
          Post-Mortem
        </h1>
        <p style={{ margin: '6px 0 0', fontSize: 13, color: '#6b7280' }}>
          Compara lo teórico (Existencia Teórica) contra lo real (Fill Rate y PedidoVSCargo Real) día a día, y calcula las diferencias.
        </p>
      </div>

      {/* Parámetros de ejecución */}
      <div style={{
        background: '#f8faff', border: '1px solid #c7d7fd', borderRadius: 14,
        padding: '22px 24px', marginBottom: 28,
      }}>
        <div style={{ fontWeight: 700, fontSize: 14, color: '#1e3a8a', marginBottom: 16 }}>
          Parámetros de ejecución
        </div>
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13, color: '#374151' }}>
            Fecha de venta
            <input type="date" value={fechaVenta} disabled={running}
              onChange={e => setFechaVenta(e.target.value)}
              style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border)', fontSize: 13, background: '#fff' }} />
          </label>
          <div style={{ fontSize: 12, color: '#6b7280', paddingBottom: 8, maxWidth: 320 }}>
            Toma automáticamente la última ejecución OK de Existencia Teórica para esa fecha.
          </div>
          <button className="btn primary" onClick={handleEjecutar}
            disabled={running || !fechaVenta}
            style={{ padding: '9px 28px', fontWeight: 700, fontSize: 14, height: 38 }}>
            {running ? '⏳ Ejecutando…' : '▶ Ejecutar'}
          </button>
        </div>

        {running && (
          <div style={{ marginTop: 16, fontSize: 13, color: '#1d4ed8' }}>
            ⏳ Calculando Post-Mortem… esto puede tardar unos minutos.
          </div>
        )}

        {result && !running && (
          <div style={{
            marginTop: 16, padding: '10px 14px', borderRadius: 8, fontSize: 13,
            background: result.ok ? '#ecfdf5' : '#fef2f2',
            color:      result.ok ? '#065f46'  : '#991b1b',
            border: `1px solid ${result.ok ? '#6ee7b7' : '#fca5a5'}`,
          }}>
            {result.ok
              ? `✓ Ejecución completada — ${result.d?.totalFilas?.toLocaleString() ?? 0} filas en ${fmtDur(result.d?.duracionMs)}`
              : `✕ ${result.msg}`}
          </div>
        )}
      </div>

      {/* Historial */}
      <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text)', marginBottom: 12 }}>
        Historial de ejecuciones
      </div>
      {loadingH ? (
        <div style={{ color: '#9ca3af', fontSize: 13 }}>Cargando historial…</div>
      ) : historial.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '32px 0', color: '#9ca3af', fontSize: 13,
          border: '1px dashed var(--border)', borderRadius: 12 }}>
          Sin ejecuciones registradas aún.
        </div>
      ) : (
        <div style={{ overflowX: 'auto', borderRadius: 12, border: '1px solid var(--border)' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: '#f9fafb' }}>
                {['Fecha venta','Ejecutado por','Ejecutado el','Duración','Filas','Estado',''].map(h => (
                  <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 600,
                    color: '#374151', borderBottom: '1px solid var(--border)', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {historial.map((row, i) => {
                const dt = row.ejecutadoEl ? new Date(row.ejecutadoEl) : null
                const enCurso = row.estado === 'ejecutando'
                const eliminando = deletingId === row.ejecucionId
                return (
                  <tr key={row.ejecucionId ?? i} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                    <td style={{ padding: '9px 14px' }}>{row.fechaVenta}</td>
                    <td style={{ padding: '9px 14px' }}>{row.usuario ?? '—'}</td>
                    <td style={{ padding: '9px 14px' }}>{dt ? dt.toLocaleString('es-MX') : '—'}</td>
                    <td style={{ padding: '9px 14px' }}>{fmtDur(row.duracionMs)}</td>
                    <td style={{ padding: '9px 14px', fontWeight: 600 }}>{row.totalFilas?.toLocaleString() ?? '—'}</td>
                    <td style={{ padding: '9px 14px' }} title={row.detalle ?? ''}>
                      <span style={{
                        display: 'inline-block', padding: '2px 10px', borderRadius: 99, fontSize: 11, fontWeight: 700,
                        background: row.estado === 'OK' ? '#dcfce7' : enCurso ? '#dbeafe' : '#fef2f2',
                        color:      row.estado === 'OK' ? '#166534' : enCurso ? '#1d4ed8' : '#991b1b',
                      }}>{row.estado}</span>
                    </td>
                    <td style={{ padding: '9px 14px', whiteSpace: 'nowrap' }}>
                      {row.estado === 'OK' && (
                        <button onClick={() => handleExportar(row.ejecucionId)}
                          style={{ background: 'none', border: '1px solid #c4b5fd', borderRadius: 6,
                            color: '#1d4ed8', cursor: 'pointer', padding: '3px 10px', fontSize: 12, marginRight: 6 }}>
                          Exportar
                        </button>
                      )}
                      <button onClick={() => handleEliminar(row.ejecucionId)} disabled={eliminando}
                        style={{ background: 'none', border: '1px solid #fca5a5', borderRadius: 6,
                          color: '#dc2626', cursor: eliminando ? 'default' : 'pointer', padding: '3px 10px', fontSize: 12,
                          opacity: eliminando ? 0.5 : 1 }}>
                        {eliminando ? '…' : 'Eliminar'}
                      </button>
                    </td>
                  </tr>
                )
              })}
            </tbody>
          </table>
        </div>
      )}
    </div>
  )
}
