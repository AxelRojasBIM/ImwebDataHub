...
function downloadTemplate() {
  ...
  a.download = 'template_productos_hub.csv'
  a.click()
}
...
export default function CatalogoProductos() {
  ...
  const loadBatches = useCallback(async () => {
    ...
    try {
      const r = await fetch(`${API}/api/productos-hub/batches`)
  ...
  const handleSave = async () => {
    ...
    try {
      for (let i = 0; i < mapped.length; i += CHUNK) {
        const res = await fetch(`${API}/api/productos-hub/sync`, {
          method: 'POST',
        ...
        saved += d.saved ?? CHUNK
        setSaveResult({ ok: true, msg: `Guardando... ${saved.toLocaleString()} / ${rows.length.toLocaleString()} productos` })
      }
      setSaveResult({ ok: true, msg: `✓ ${saved.toLocaleString()} productos guardados.` })
  ...
  const handleView = async (batch) => {
    ...
    setLoadingView(true)
    try {
      const r = await fetch(`${API}/api/productos-hub/batches/${batch.batchId}`)
  ...
  const handleDelete = async (batchId) => {
    if (!confirm('¿Eliminar esta carga de productos?')) return
    await fetch(`${API}/api/productos-hub/batches/${batchId}`, { method: 'DELETE' })
  ...
  const handleDownload = async (batch) => {
    ...
    const csv = [COLS.join(','), ...data.map(r =>
      [r.orgCode,r.itemCode,r.masterCode,r.shortName,r.longName,r.price,r.barcode,
    ...
    a.download = `productos_hub_${batch.batchId.slice(0,8)}.csv`
    a.click()
  }
  ...
  return (
    <>
      <div className="topbar">
        <div>
          <div className="topbar-title">Productos HubPedidos</div>
          <div className="topbar-sub">Catálogo de productos exportado desde HubPedidos (item master)</div>
        </div>
      ...
      <div className="content">
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <div>
                ...
                <span style={{ marginLeft: 10, fontSize: 12, color: '#6b7280' }}>{rows.length.toLocaleString()} productos</span>
              </div>
            ...
            <button className="btn primary" style={{ width: '100%', justifyContent: 'center', padding: '9px 0' }} onClick={handleSave} disabled={saving}>
              {saving ? 'Guardando...' : `☁ Guardar ${rows.length.toLocaleString()} productos en la base de datos`}
            </button>
          </div>
        )}
        ...
        {!file && !saveResult && (
          ...
          >
            ...
            <div style={{ fontSize: 12, color: '#6b7280', marginTop: 5 }}>Exporta desde HubPedidos → Item Master · {COLS.length} columnas</div>
          </div>
        )}
          ...
          <div style={{ marginTop: 20, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
            ...
            {loadingView ? <div className="loading">Cargando...</div> : (
              <div style={{ maxHeight: 360, overflowY: 'auto', overflowX: 'auto' }}>
                <table>
                  <thead><tr>{PREVIEW_COLS.map(c => <th key={c}>{c}</th>)}<th>LongName</th><th>ShelfLife</th><th>CategoryItem</th></tr></thead>
                  <tbody>
                    {viewRows.slice(0, 200).map((r, i) => (
                      <tr key={i}>
                        ...
                        <td>{r.shortName}</td>
                        <td>{r.brand}</td>
                        <td>{r.categoryItem}</td>
                        <td style={{ textAlign: 'right' }}>{r.price}</td>
                        <td style={{ maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.longName}</td>
                        <td>{r.shelfLife}</td>
                        <td>{r.categoryItem}</td>
                      </tr>
                    ))}
