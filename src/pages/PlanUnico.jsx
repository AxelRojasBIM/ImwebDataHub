...
const COLS = [
  'anio', 'semana', 'cod_ceve', 'canal', 'item', 'plan_unico_irrestricto', 'plan_unico_restricciones',
]
const EJEMPLO = [
  '2026', '3', '20279', 'Detalle', '12345', '1000', '850',
]
const UPLOAD_URL  = '/api/plan-unico/upload'
const BATCHES_URL = '/api/plan-unico/batches'
const DELETE_URL  = '/api/plan-unico/batches'

const REGLAS = [
  'Ninguna columna puede quedar vacía.',
  '\'plan_unico_irrestricto\' y \'plan_unico_restricciones\' deben ser números válidos; no hay regla de mínimo, 0 es un valor aceptado.',
  ...
  'No se permiten filas duplicadas por anio + semana + cod_ceve + canal + item.',
]
...
function descargarPlantilla() {
  ...
  a.href = url; a.download = 'plantilla_plan_unico.csv'
  document.body.appendChild(a); a.click(); document.body.removeChild(a)
  URL.revokeObjectURL(url)
}

const ROWS_COLS = [
  ['anio', 'anio'], ['semana', 'semana'], ['codCeve', 'cod_ceve'], ['canal', 'canal'],
  ['codCanal', 'cod_canal'], ['item', 'item'],
  ['planUnicoIrrestricto', 'plan_unico_irrestricto'], ['planUnicoRestricciones', 'plan_unico_restricciones'],
]
...
async function exportarBatchExcel(batchId, nombreArchivo) {
  ...
  const ws = wb.addWorksheet('Plan Único', { views: [{ state: 'frozen', ySplit: 1 }] })
  ...
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url; a.download = `${(nombreArchivo || 'plan_unico').replace(/\.csv$/i, '')}.xlsx`
  document.body.appendChild(a); a.click(); document.body.removeChild(a)
  URL.revokeObjectURL(url)
}
...
export default function PlanUnico() {
